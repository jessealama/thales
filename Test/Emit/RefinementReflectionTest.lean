/-
  ...
  Exercises predicates, throwing constructors, the boundary axioms,
  and the homomorphism theorems with concrete inputs so a missing or
  renamed definition fails the test. Must be free of unfinished
  proofs.
...
-- Boundary axioms — referenced by name (rename = test fails).
#check @Float.ofInt_neg
#check @Float.ofInt_lt
#check @Float.ofInt_le
#check @Nat.toFloat_isSafeInteger
#check @Float.neg_isSafeInteger
