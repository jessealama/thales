...
/-- Replace `.ref name []` with `.typeVar varId name constraint` throughout a type.
    Used to convert type parameter name references in a generic body to
    unique type variables before substitution.
    The constraint is embedded in the typeVar so call-site inference can check it. -/
...
/-- Substitute type variables in a TSType according to bindings map -/
...
/-- Replace all occurrences of .ref name [] with a replacement type.
...
/-- Replace all structural occurrences of `from_` with `to_` in `ty`.
    Used for per-member substitution during conditional type distribution. -/
private partial def substituteType (ty from_ to_ : TSType) : TSType :=
...
/-- Build a name → bound-type mapping for infer variables found in extends_.
    Infer variables appear as .typeVar id name _ with id >= 9000 in the extends pattern.
    After collectTypeVarBindings, the bindings map id → concrete type.
    This function produces a list of (name, concrete type) pairs for ref substitution. -/
private partial def buildInferNameBindings (extends_ : TSType)
...
private partial def substituteInferRefs (ty : TSType) (nameBindings : List (String × TSType)) : TSType :=
  match ty with
  | .ref name [] =>
