...
`Thales.TS.Runtime` postulates thirteen `axiom`s. They all encode
...
refinement types and provably-safe indexing their soundness story.
...
Soundness basis for `Integer.ofInt`, `Integer.toInt`, `Natural.toNat`,
and the in-range `Int` ↔ `Float` round-trip.
...
| `Float.toUInt64_of_isNatural` | A Float satisfying `isNatural` round-trips through `Float → UInt64 → Nat → Float`. | Soundness of `Natural.toNat`                                     |
...
The first three were validated against the `feat/thales-grind-poc`
branch before v0.6 landed; the remaining four are introduced for v0.6.
...
trusting (in addition to Lean's own kernel and stdlib) thirteen
