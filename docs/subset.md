...
function increment(c: Counter): Counter { return { count: c.count + 1 }; }
function getCount(c: Counter): number { return c.count; }
```
...
The example corpus verifies that the VM execution and the compiled Lean both produce the same output on the corpus inputs. This is *behavioral* equivalence on a finite set of programs, not a proof of *semantic* equivalence. A program not in the corpus that exercises an unhandled edge case can diverge silently between the two paths. A real semantic-equivalence theorem is 5 work.
