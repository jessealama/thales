...
v0.6 adds four prelude refinement types (`Integer`, `Natural`, `Byte`, `Bit`) and provably-safe array indexing. See the dedicated sections below for details.
...
## Provably-safe array indexing (0.6)

Standard TypeScript array indexing (`arr[i]`) returns `T | undefined` (with `noUncheckedIndexedAccess`) or `T` (without). In either case the type-level guarantee is weak: TypeScript does not know whether `i` is a valid index. Lean's `Array.get` requires a proof that `i < arr.size`.

Thales 0.6 provides two patterns that let the compiler emit a _proof-carrying_ index expression rather than a partial lookup:

### P1 — Literal indexing

When the index is a numeric literal and the array is a literal tuple, the compiler can verify at type-check time that the index is in bounds. Thales emits `arr[i]!` (with a compile-time proof obligation discharged by `decide`).

```typescript
const pair = [10, 20] as const;
const first = pair[0]; // safe — literal index into literal tuple
```

### P2 — Length-narrowed indexing

When a loop variable or index is narrowed by a `Natural`-typed bound that is exactly `arr.length`, Thales can emit a proof that the index is in bounds. The idiomatic pattern:
...
function sumAll(arr: number[]): number {
  let total = 0;
  const len: Natural = arr.length; // arr.length is Natural
  for (let i: Natural = 0; i < len; i = asNatural(i + 1)) {
    total += arr[i]; // safe — i < arr.length proven
  }
  return total;
}
```
