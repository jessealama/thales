...
| Records      | `interface` and single-record `type` aliases (interchangeable) — nominal by declaration; constructed with object literals (`{ x, y }` / `{ x: e }`) where the target structure is known: return, annotated-local, call-argument, and array-element positions. Literals with no resolvable named-structure target are out of subset. |
